            ...
            {product ? (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed bottom-0 left-0 right-0 z-50 w-full max-h-[90vh] overflow-y-auto rounded-t-[2.5rem] bg-white"
                        initial={{ y: "100%" }}
                        animate={{ y: 0 }}
                        exit={{ y: "100%" }}
                    ...
                    >
                        <div className="mx-auto w-full max-w-2xl px-4 pb-6">
                            <div className="mx-auto mb-2 mt-4 h-1.5 w-12 rounded-full bg-gray-300" />
                            ...
                            <div className="relative mt-4 h-64 w-full overflow-hidden rounded-2xl bg-slate-100">
                                ...
                                <Button
                                    ...
                                    size="icon"
                                    onClick={onClose}
                                    className="absolute right-4 top-4 h-10 w-10 rounded-full bg-white/90 text-slate-900 shadow-lg backdrop-blur hover:bg-white"
                            ...
                            <div className="space-y-6 pt-6">
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <p className="mb-1 text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">
                                            Öne Çıkan Lezzet
                                        </p>
                                        <h2 id="product-modal-title" className="text-2xl font-bold tracking-tight text-slate-900">
                                            {product.name}
                                        </h2>
                                        {product.isSoldOut && (
                                            <span className="mt-2 inline-flex rounded-full bg-slate-900/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-700">
                                                Tükendi
                                            </span>
                                        )}
                                    </div>

                                    <div className={`rounded-2xl px-4 py-2 text-lg font-bold shadow-sm ${product.isSoldOut ? "bg-slate-200 text-slate-600" : "bg-slate-900 text-white"}`}>
                                        ₺{product.price}
                                    </div>
                                </div>
                                ...
                                {product.badges && product.badges.length > 0 && (
                                    <div className="flex flex-wrap gap-2">
                                        {product.badges.map((badge) => (
                                            <span
                                                key={badge}
                                                className="inline-flex items-center gap-1.5 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700"
                                            >
                                                {getBadgeIcon(badge)}
                                                {badge}
                                            </span>
                                ...
                                <section className="space-y-3">
                                    <h3 className="text-sm font-semibold text-slate-900">Besin Değerleri</h3>
                            ...
                            <div className="sticky bottom-0 mt-6 border-t border-slate-200 bg-white/95 py-4 backdrop-blur">
                                <Button
                                    ...
                                    className="h-14 w-full rounded-2xl text-base font-semibold shadow-lg shadow-slate-200/80 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none"
                                >
